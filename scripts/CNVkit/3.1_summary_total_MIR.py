...
def summarize_directory(input_dir, output_csv):
    summary_rows = []
    ...
    for trio in trios:
        ...
        cur_MIR_file = os.path.join(cur_path, "mendelian_consistency.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-dir", required=True, help="Directory containing per-trio mendelian_consistency.csv files")
    parser.add_argument("--output", required=True, help="Path to output summary CSV")
    args = parser.parse_args()

    summarize_directory(args.input_dir, args.output)
